    ...
    \begin{abstract}
    Visual speech recognition (VSR), commonly called lip reading, addresses speech understanding when audio is weak, unavailable, or intentionally excluded. This direction is increasingly important for accessibility systems, subtitle reconstruction, and multimodal perception pipelines that must remain reliable in difficult recording conditions. Yet VSR is still constrained by a core scientific challenge: multiple phonemes can map to similar visible mouth motions, and this ambiguity grows when videos contain realistic noise such as motion blur, camera compression, occlusion, low light, and viewpoint shifts. As a result, many models that achieve strong benchmark numbers under controlled capture conditions fail to maintain quality in the open world.

    This paper studies robust VSR for noisy environments, with emphasis on practical subtitle reconstruction and transparent model behavior. Our approach builds from two key anchors. First, we use LipNet~\cite{assael2016lipnet} as a foundational reference for end-to-end sentence-level visual decoding with sequence modeling and CTC alignment. Second, we follow the central architectural intuition of VALLR~\cite{thomas2025vallr}: predicting intermediate phoneme-like representations from video and using language-level reconstruction to convert noisy intermediate sequences into coherent text. We reframe these ideas into a modular, reproducible pipeline that includes stabilized mouth-region preprocessing, a multi-scale transformer temporal encoder, top-$k$ CTC intermediate decoding, and noise-aware language correction via a LoRA-fine-tuned language model~\cite{hu2022lora}.

    We additionally include an ethics-centered discussion of privacy, misuse, demographic imbalance, and uncertainty communication; an explicit limitations section; and a future-work agenda that prioritizes robust deployment and participatory evaluation with deaf and hard-of-hearing users.
    \end{abstract}
    ...
    This project focuses on that gap between ``works in clean benchmarks'' and ``works in realistic environments.'' Rather than treating this as a purely architectural competition, we frame it as a system-design problem: robust preprocessing, temporal representation, intermediate decoding strategy, language correction behavior, and confidence calibration all matter. We argue that progress should be measured not only by best-case Word Error Rate (WER), but by degradation slope under controlled perturbations, interpretability of failures, and usability of uncertainty outputs in assistive contexts.
    ...
    These questions are intentionally tied to practice. The goal is not only to maximize a metric, but to understand how and why model behavior changes as environments become more realistic.
    ...
    \end{equation}
    This perspective makes explicit that robustness is not an afterthought metric but part of the core optimization objective and architecture design rationale.
    ...
    \begin{enumerate}[leftmargin=*]
        \item A practical VSR framework combining robust preprocessing, multi-scale Conformer-style temporal modeling~\cite{conformer2020}, CTC intermediate prediction, and LoRA-fine-tuned language-level reconstruction~\cite{hu2022lora}.
        \item A story-driven evaluation scaffold that prioritizes degradation behavior under perturbation, not only clean-condition ranking.
        \item A readable Results and Discussion section written to stand independently of figures while defining concrete visualization placeholders.
        \item An ethics-grounded interpretation that connects empirical trends to accessibility utility, misuse risk, and mitigation design.
    \end{enumerate}
    ...
    Given clip $\mathbf{X}=\{x_t\}_{t=1}^{T}$, we estimate a mouth-centered sequence $\mathbf{M}=\{m_t\}_{t=1}^{T}$ through detection, tracking, normalization, and temporal smoothing. Let $b_t$ denote the ROI box at frame $t$. We smooth trajectories with:
    \begin{equation}
    \bar{b}_t = \alpha b_t + (1-\alpha)\bar{b}_{t-1},
    \end{equation}
    which reduces high-frequency crop jitter that otherwise amplifies temporal modeling noise.

    This design is motivated by a practical observation: in VSR, unstable crops create synthetic motion that can be confounded with articulation. Many underperforming pipelines fail not because temporal models are weak, but because ROI instability corrupts input statistics.
    ...
    \end{equation}
    Our primary model uses a VideoMAE-style spatiotemporal ViT~\cite{tong2022videomae} with 3D tubelet embedding (tubelet size 2, patch size 16$\times$16, embed dim 768). The backbone uses 8 transformer blocks with multi-head self-attention and follows the masked autoencoder pre-training paradigm~\cite{tong2022videomae} to enable data-efficient learning.

    The architecture choice is therefore not ``transformers are always better.'' It is a bias-variance tradeoff: stronger global modeling is useful when regularized by robust preprocessing and augmentation, otherwise gains can collapse under shift.
    ...
    Temporal transformers are chosen in the primary model because viseme ambiguity is frequently resolved by distant context rather than local frame evidence. For example, similar lip closures can correspond to multiple phoneme classes, and disambiguation often requires broader utterance context. Conformer-style blocks~\cite{conformer2020} are particularly effective here as they combine self-attention for global context with depthwise convolution for local articulation dynamics.
    ...
    \begin{figure*}[t]
    \centering
    \includegraphics[width=0.98\textwidth]{topk_phonemes.pdf}
    ...
    This choice is expected to reduce catastrophic substitution errors caused by premature hard decisions. It also improves interpretability, since low-confidence alternatives can be inspected when the final prediction is uncertain.
    ...
    \caption{Noise-aware phoneme-to-text fine-tuning pipeline. Training text (WikiText-2) is converted to clean ARPAbet phoneme sequences via CMUdict. The \texttt{PhonemeNoiseAugmenter} (substitution $p=0.15$, insertion $p=0.05$, deletion $p=0.05$) applies viseme-aware confusions (e.g., B$\leftrightarrow$P$\leftrightarrow$M, F$\leftrightarrow$V) to simulate realistic CTC decoding errors. LLaMA-3.2-3B is fine-tuned via LoRA~\cite{hu2022lora} (rank 16, $\alpha=32$) to map noisy phoneme inputs back to original clean text. Setting \texttt{include\_clean=True} doubles the effective training set so the model remains anchored to clean phoneme mappings while learning to correct errors.}
    \label{fig:finetune}
    \end{figure*}
    ...
    We use LoRA~\cite{hu2022lora} to fine-tune LLaMA-2~\cite{touvron2023llama2} on this phoneme-to-text task with rank $r=16$ and $\alpha=32$, targeting all attention projection matrices (\texttt{q\_proj}, \texttt{k\_proj}, \texttt{v\_proj}, \texttt{o\_proj}) and MLP gate/up/down projections. This reduces trainable parameters by approximately four orders of magnitude relative to full fine-tuning while maintaining strong generalization.

    This training strategy is motivated directly by the two-stage VALLR principle~\cite{thomas2025vallr}: the second stage should \emph{repair} noisy intermediate predictions, not merely translate clean symbols. In practice, noise-aware training makes the reconstructor less brittle and improves its ability to recover intended text from plausible but imperfect phoneme paths.
    ...
    \end{equation}
    After concatenation-and-projection fusion, a Transformer (or Conformer~\cite{conformer2020}) context stack adds global temporal reasoning before a strided convolutional downsampling stage.
    ...
    VSR has clear potential for social benefit, especially for deaf and hard-of-hearing communication support, noisy classroom captioning, and accessibility in low-audio contexts. However, the same capability can be misused for non-consensual surveillance or private speech inference from video.
    ...
    Why benefits may outweigh drawbacks: under controlled, consent-based deployments with uncertainty-aware interfaces and oversight, VSR can improve communication access for communities historically underserved by speech technology. The mitigation strategy is therefore not optional post-processing but a core requirement for responsible use.
    ...
    \begin{itemize}[leftmargin=*]
        ...
        \item language reconstruction can hide upstream uncertainty and occasionally produce plausible but incorrect text,
        \item compute and latency costs can limit real-time or edge deployment,
    ...
    These directions align directly with the practical mission of accessibility-oriented VSR: reliable operation under noise, transparent uncertainty, and real user utility.

    \section{Conclusion}
    This paper investigates robust visual speech recognition for subtitle reconstruction in noisy and unconstrained environments. Building on the foundational sequence perspective of LipNet~\cite{assael2016lipnet} and the staged phoneme-to-text decomposition emphasized by VALLR~\cite{thomas2025vallr}, we implemented and analyzed a modular pipeline that integrates stabilized preprocessing, VideoMAE-style spatiotemporal ViT encoding~\cite{tong2022videomae}, multi-scale Conformer temporal modeling~\cite{conformer2020}, top-$k$ CTC uncertainty transfer (Figure~\ref{fig:topk}), and noise-aware LLaMA LoRA reconstruction (Figure~\ref{fig:finetune}).
